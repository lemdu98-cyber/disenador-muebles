...
export default function Wardrobe({ height, depth, thickness = .015, backThickness = .003, drawerDimensions, drawerFrontConfig, structure }) {
  ...
  return <group>
    ...
    {cfg.showDoors && !cfg.showStructure && (s.isSlidingDoors
      ...
      : <>{s.bodyCentersXCm.map((x, index) => <Door key={`upper-door-${index}`} centerX={x / 100} centerY={s.upperDoorCenterYCm / 100} width={s.doorWidthCm / 100} height={s.upperDoorHeightCm / 100} depth={depth} thickness={thickness} open={cfg.showOpenDoors} />)}
        {s.bodyCentersXCm.map((x, index) => <Door key={`main-door-${index}`} centerX={x / 100} centerY={s.mainDoorCentersYCm[index] / 100} width={s.doorWidthCm / 100} height={s.mainDoorHeightsCm[index] / 100} depth={depth} thickness={thickness} open={cfg.showOpenDoors} handleY={index === 1 ? 0 : -s.mainDoorHeightsCm[index] / 400} />)}</>)}
  </group>;
}
