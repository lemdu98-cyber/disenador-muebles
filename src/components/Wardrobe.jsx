...
export default function Wardrobe({ height, depth, thickness = .015, backThickness = .003, drawerDimensions, drawerFrontConfig, structure }) {
  ...
  return <group>
    ...
    {cfg.showDoors && !cfg.showStructure && (s.isSlidingDoors
      ...
      : s.bodyCentersXCm.map((x, index) => <Door key={`door-${index}`} index={index} centerX={x / 100} width={s.doorWidthCm / 100} height={s.doorHeightCm / 100} depth={depth} thickness={thickness} open={cfg.showOpenDoors} />))}
  </group>;
}
