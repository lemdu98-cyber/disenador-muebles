...
export default function Nightstand({ width, height, depth, drawers, thickness = .015, backThickness = .003, drawerDimensions, drawerFrontConfig, structure, manufacturingPieces = [], highlightedComponentIds }) {
  ...
  return <group>
    <MelaminePanel position={[0, height / 2 - thickness / 2, thickness / 2]} dimensions={[(topPiece?.length ?? width * 100) / 100, thickness, topDepth]} piece={topPiece} orientation="horizontal" color={TOP} highlighted={highlightedComponentIds?.has("nightstand.top")} />
    {[-1, 1].map((side, index) => <MelaminePanel key={side} position={[side * (width / 2 - thickness / 2), -thickness / 2, 0]} dimensions={[thickness, usableHeight, (cut("Laterales", index)?.width ?? depth * 100) / 100]} piece={cut("Laterales", index)} orientation="side" color={MELAMINE} highlighted={highlightedComponentIds?.has(index ? "nightstand.rightSide" : "nightstand.leftSide")} />)}
    <mesh position={[0, 0, backPanel.centerZ]}><boxGeometry args={[(backPiece?.length ?? backPanel.width * 100) / 100, (backPiece?.width ?? backPanel.height * 100) / 100, backPanel.thickness]} /><meshStandardMaterial color={HARDBOARD} /><Edges color="#49382d" threshold={15} scale={1.001} />{highlightedComponentIds?.has("nightstand.back") && <SelectionHighlight dimensions={[(backPiece?.length ?? backPanel.width * 100) / 100, (backPiece?.width ?? backPanel.height * 100) / 100, backPanel.thickness]} />}</mesh>
    {structure.config.rearEnabled && <MelaminePanel position={[0, -height / 2 + rearHeight / 2, -depth / 2 + thickness / 2]} dimensions={[(rearPiece?.length ?? (width - thickness * 2) * 100) / 100, rearHeight, thickness]} piece={rearPiece} orientation="front" color={MELAMINE} highlighted={highlightedComponentIds?.has("nightstand.rearCrossbar")} />}
