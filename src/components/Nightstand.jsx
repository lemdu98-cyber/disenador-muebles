...
export default function Nightstand({ width, height, depth, drawers, thickness = .015, backThickness = .003 }) {
  ...
  return <group>
    ...
    {Array.from({ length: drawers }).map((_, index) => <Drawer key={index} width={width - thickness * 2 - .012} height={drawerHeight - .012} depth={depth - thickness * 2} thickness={thickness} baseThickness={backThickness} position={[0, height / 2 - thickness - drawerHeight / 2 - index * drawerHeight, 0]} />)}
