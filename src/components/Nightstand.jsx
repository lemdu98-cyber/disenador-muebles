...
export default function Nightstand({ width, height, depth, drawers, thickness = .015, backThickness = .003, drawerDimensions, drawerFrontConfig, structure }) {
  ...
  return <group>
    <mesh position={[0, height / 2 - thickness / 2, 0]}><boxGeometry args={[width, thickness, depth]} /><meshStandardMaterial color={TOP} /><Edges color="#49382d" threshold={15} scale={1.001} /></mesh>
    ...
    {drawerDimensions.hasEnoughDepth && structure.valid && Array.from({ length: drawers }).map((_, index) => <Drawer key={index} width={drawerDimensions.externalWidthCm / 100} height={drawerHeight - .012} depth={drawerDimensions.sideLengthCm / 100} thickness={thickness} baseThickness={backThickness} drawerFrontConfig={drawerFrontConfig} showEdges position={[0, height / 2 - thickness - drawerHeight / 2 - index * drawerHeight, 0]} />)}
