...
export default function Nightstand({ width, height, depth, drawers, thickness = .015, backThickness = .003, drawerDimensions, drawerFrontConfig, structure, manufacturingPieces = [], highlightedComponentIds, onSelectComponent }) {
  ...
  return <group>
    <MelaminePanel position={[0, height / 2 - thickness / 2, thickness / 2]} dimensions={[(topPiece?.length ?? width * 100) / 100, thickness, topDepth]} piece={topPiece} orientation="horizontal" color={TOP} highlighted={highlightedComponentIds?.has("nightstand.top")} componentId="nightstand.top" onSelectComponent={onSelectComponent} />
    {[-1, 1].map((side, index) => { const id = index ? "nightstand.rightSide" : "nightstand.leftSide"; return <MelaminePanel key={side} position={[side * (width / 2 - thickness / 2), -thickness / 2, 0]} dimensions={[thickness, usableHeight, (cut("Laterales", index)?.width ?? depth * 100) / 100]} piece={cut("Laterales", index)} orientation="side" color={MELAMINE} highlighted={highlightedComponentIds?.has(id)} componentId={id} onSelectComponent={onSelectComponent} />; })}
    <mesh position={[0, 0, backPanel.centerZ]} onClick={(event) => { event.stopPropagation(); onSelectComponent?.("nightstand.back"); }}><boxGeometry args={[(backPiece?.length ?? backPanel.width * 100) / 100, (backPiece?.width ?? backPanel.height * 100) / 100, backPanel.thickness]} /><meshStandardMaterial color={HARDBOARD} /><Edges color="#49382d" threshold={15} scale={1.001} />{highlightedComponentIds?.has("nightstand.back") && <SelectionHighlight dimensions={[(backPiece?.length ?? backPanel.width * 100) / 100, (backPiece?.width ?? backPanel.height * 100) / 100, backPanel.thickness]} />}</mesh>
    {structure.config.rearEnabled && <MelaminePanel position={[0, -height / 2 + rearHeight / 2, -depth / 2 + thickness / 2]} dimensions={[(rearPiece?.length ?? (width - thickness * 2) * 100) / 100, rearHeight, thickness]} piece={rearPiece} orientation="front" color={MELAMINE} highlighted={highlightedComponentIds?.has("nightstand.rearCrossbar")} componentId="nightstand.rearCrossbar" onSelectComponent={onSelectComponent} />}
    {structure.config.frontEnabled && structure.valid && <MelaminePanel position={[0, -height / 2 + frontHeight / 2, depth / 2 - thickness / 2]} dimensions={[(frontPiece?.length ?? (width - thickness * 2) * 100) / 100, frontHeight, thickness]} piece={frontPiece} orientation="front" color={MELAMINE} highlighted={highlightedComponentIds?.has("nightstand.frontCrossbar")} componentId="nightstand.frontCrossbar" onSelectComponent={onSelectComponent} />}
    {drawerDimensions.hasEnoughDepth && structure.valid && structure.drawerGeometry.drawerLayouts.map((layout) => <group key={layout.index}>
      <Drawer width={drawerBoxWidth} height={layout.boxHeightCm / 100} depth={drawerDepth} thickness={thickness} baseThickness={structure.drawerGeometry.bottomThicknessCm / 100} drawerFrontConfig={drawerFrontConfig} frontDimensions={{ widthCm: structure.drawerFrontWidthCm, heightCm: layout.frontHeightCm }} manufacturingDimensions={drawerCuts(layout.index)} sideHeightOverride={layout.sideHeightCm / 100} physicalGeometry={{ frontCenterY: layout.frontCenterYCm / 100, structureCenterY: layout.structureCenterYCm / 100, bottomCenterY: layout.bottomCenterYCm / 100 }} showEdges position={[0, 0, closedDrawerCenterZ + drawerOpenOffset]} drawerComponentId={`nightstand.drawer.${layout.index + 1}`} highlightedComponentIds={highlightedComponentIds} onSelectComponent={onSelectComponent} />
