...
import { getDrawerComponentIds } from "../utils/furnitureModel";
...
export default function Drawer({
  ...
  const ids = drawerComponentId ? getDrawerComponentIds(drawerComponentId) : null;
  const highlighted = (id) => Boolean(id && highlightedComponentIds?.has(id));

  return <group position={position}>
    <MelaminePanel position={[geometry.front.position.xCm / 100, geometry.front.position.yCm / 100, geometry.front.position.zCm / 100]} dimensions={[geometry.front.dimensions.widthCm / 100, geometry.front.dimensions.depthCm / 100, geometry.front.dimensions.heightCm / 100]} piece={manufactured.frontPiece} orientation="front" color={MELAMINE} showEdges={showEdges} highlighted={highlighted(ids?.front)} componentId={ids?.front} onSelectComponent={onSelectComponent} />
    <MelaminePanel position={[geometry.leftSide.position.xCm / 100, geometry.leftSide.position.yCm / 100, geometry.leftSide.position.zCm / 100]} dimensions={[geometry.leftSide.dimensions.heightCm / 100, geometry.leftSide.dimensions.widthCm / 100, geometry.leftSide.dimensions.depthCm / 100]} piece={manufactured.sideLeftPiece} orientation="side" color={MELAMINE} showEdges={showEdges} highlighted={highlighted(ids?.leftSide)} componentId={ids?.leftSide} onSelectComponent={onSelectComponent} />
    <MelaminePanel position={[geometry.rightSide.position.xCm / 100, geometry.rightSide.position.yCm / 100, geometry.rightSide.position.zCm / 100]} dimensions={[geometry.rightSide.dimensions.heightCm / 100, geometry.rightSide.dimensions.widthCm / 100, geometry.rightSide.dimensions.depthCm / 100]} piece={manufactured.sideRightPiece} orientation="side" color={MELAMINE} showEdges={showEdges} highlighted={highlighted(ids?.rightSide)} componentId={ids?.rightSide} onSelectComponent={onSelectComponent} />
    <MelaminePanel position={[geometry.back.position.xCm / 100, geometry.back.position.yCm / 100, geometry.back.position.zCm / 100]} dimensions={[geometry.back.dimensions.widthCm / 100, geometry.back.dimensions.depthCm / 100, geometry.back.dimensions.heightCm / 100]} piece={manufactured.backPiece} orientation="front" color={MELAMINE} showEdges={showEdges} highlighted={highlighted(ids?.back)} componentId={ids?.back} onSelectComponent={onSelectComponent} />
    <mesh position={[geometry.bottom.position.xCm / 100, geometry.bottom.position.yCm / 100, geometry.bottom.position.zCm / 100]} onClick={ids?.bottom ? (event) => { event.stopPropagation(); onSelectComponent?.(ids.bottom); } : undefined}><boxGeometry args={[geometry.bottom.dimensions.widthCm / 100, geometry.bottom.dimensions.heightCm / 100, geometry.bottom.dimensions.depthCm / 100]} /><meshStandardMaterial color={HARDBOARD} />{showEdges && <Edges color="#62462f" threshold={15} scale={1.002} />}{highlighted(ids?.bottom) && <SelectionHighlight dimensions={[geometry.bottom.dimensions.widthCm / 100, geometry.bottom.dimensions.heightCm / 100, geometry.bottom.dimensions.depthCm / 100]} />}</mesh>
