...
export default function DrawerFrontSettings({ config, onChange, disabled, boxWidthCm }) {
  ...
  return <section className="configuration drawer-front-settings">
    ...
    <label className="check-setting"><input type="radio" name="drawer-front-type" checked={config.type === "overlay"} onChange={() => update({ type: "overlay" })} disabled={disabled} />Frente superpuesto</label>
    <label className="check-setting"><input type="radio" name="drawer-front-type" checked={config.type === "inset"} onChange={() => update({ type: "inset" })} disabled={disabled} />Frente interior</label>
    <div className="field-grid">
      <label>Solape lateral (cm)<input type="number" min="0" step="0.01" value={config.sideOverlayCm} disabled={disabled || config.type !== "overlay"} onChange={numericUpdate("sideOverlayCm")} /></label>
    ...
    <div className="front-width-summary"><span>Ancho de la caja</span><b>{boxWidthCm.toFixed(2)} cm</b><span>Ancho del frente</span><b>{front.widthCm.toFixed(2)} cm</b></div>
  </section>;
}
