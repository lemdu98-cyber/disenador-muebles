...
export default function App() {
  ...
  const [nightstandCrossbarHeightCm, setNightstandCrossbarHeightCm] = useState(8);
  ...
  const design = { furnitureType, widthCm, heightCm, depthCm, doors, drawers, shelves, drawerSlideConfig, drawerFrontConfig, catHouseConfig, nightstandCrossbarHeightCm, drawerValidationError };
  ...
  return <main className="app-shell">
    <aside className="control-panel">
      ...
      <section className="configuration">
        <h2>Configuración</h2>
        {isCatHouse ? <p className="configuration-note">Estructura abierta sin puertas ni cajones.</p> : isDesk || isNightstand ? <>
          <label>Cajones<input type="number" min="0" max="6" value={drawers} onChange={updateDimension(setDrawers, 0)} /></label>
          {isNightstand && <label>Altura del travesaño (cm)<input type="number" min="1" max={Math.max(1, heightCm - melamineThickness * 100)} step="0.1" value={nightstandCrossbarHeightCm} onChange={(event) => setNightstandCrossbarHeightCm(Math.min(Math.max(1, heightCm - melamineThickness * 100), Math.max(1, Number(event.target.value) || 1)))} /></label>}
        </> : <>
